  ...
  private async getContentId(
    ...
    try {
      ...
      if (res.data?.items && res.data.items.length > 0) {
        ...
        console.log(
          `[MIGU] Found contentId via audioPlayer/songs: ${item.contentId}`,
        );
        ...
        try {
          ...
          if (data) {
            ...
            if (data.contentId) {
              ...
              console.log(
                `[MIGU] Found contentId in top-level resource data: ${contentId}`,
              );
            } else if (data.songId) {
              ...
              const targetFormat = formats.find((f: any) => f.contentId);
              if (targetFormat) {
                contentId = targetFormat.contentId;
                console.log(
                  `[MIGU] Found contentId via resourceinfo.do formats: ${contentId}`,
                );
              } else {
                console.log(
                  `[MIGU] No contentId found in resourceinfo.do formats. Formats available: ${formats.length}`,
                );
                if (formats.length > 0) {
                  console.log(
                    `[MIGU] First format keys:`,
...
-----END PUBLIC KEY-----`;
        ...
        if (webRes.status === 200 && !webRes.data?.data?.playUrl) {
          console.log(
            `[MIGU] Web API Response Data (Debug):`,
      ...
      if (contentId) {
        ...
        try {
          ...
          if (pcRes.data?.data?.url) {
            ...
            console.log(
              `[MIGU] PC Strategy API Response (Debug):`,
              JSON.stringify(pcRes.data, null, 2),
            );
          }

          // Try HQ if SQ fails
          if (!pcRes.data?.data?.url) {
            const pcResHQ = await axios.get(
              "https://app.c.nf.migu.cn/MIGUM3.0/strategy/pc/listen/v1.0",
              {
                params: {
                  copyrightId,
      ...
      for (const formatType of ["SQ", "HQ", "PQ"]) {
        const format = formats.find((f: any) => f.formatType === formatType);
        if (format) {
          const url = format.url || format.androidUrl;
          if (url) {
            foundUrl = url.replace(
              /ftp:\/\/[^/]+/,
              "https://freetyst.nf.migu.cn",
            );
            console.log(
              `[MIGU] Found URL in resource info for ${formatType}: ${foundUrl}`,
            );
            break;
          }
        }
      ...
      if (targetFormat && targetContentId) {
        try {
          console.log(
            `[MIGU] Calling listenSong.do with contentId: ${targetContentId}...`,
          );
      ...
      try {
        const listenUrlRes = await axios.get(
        ...
        console.log(
          `[MIGU] Listen URL API Data:`,
          JSON.stringify(listenUrlRes.data, null, 2),
        );

        if (
          listenUrlRes.data?.code === "000000" &&
        ...
        if (listenUrlRes.data?.code !== "000000") {
          ...
          const listenUrlResHQ = await axios.get(
            "https://app.c.nf.migu.cn/MIGUM2.0/v2.0/content/listen-url",
            {
              params: {
                copyrightId,
          ...
          if (
            listenUrlResHQ.data?.code === "000000" &&
