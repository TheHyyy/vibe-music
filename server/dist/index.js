...
app.post("/api/rooms/:roomId/queue", (req, res) => {
    try {
        ...
        }).catch((e) => console.error("[EventLog] song.requested failed", e, "cwd=", process.cwd()));
        ...
        if (rec.nowPlaying?.id !== oldNowPlayingId) {
            if (rec.nowPlaying) {
                broadcastSystemMessage(roomId, `切歌: ${rec.nowPlaying.song.title} - ${rec.nowPlaying.song.artist || "未知歌手"}`);
            }
    ...
    try {
        ...
        if (rec) {
            ...
            if (input.type === "SKIP" &&
                ...
                if (nowPlaying) {
                    broadcastSystemMessage(roomId, `投票跳过成功，开始播放: ${nowPlaying.song.title}`);
                }
...
app.post("/api/rooms/:roomId/admin/next", async (req, res) => {
    ...
    try {
        ...
        if (currentSongId &&
            ...
            console.log(`[AdminNext] Skipped redundant request. Client: ${currentSongId}, Server: ${rec.nowPlaying.id}`);
...
io.on("connection", (socket) => {
    socket.on("room:join", (body, ack) => {
        try {
            ...
            if (wasScheduled) {
                console.log(`[Join] Canceled pending leave for user ${userId} in room ${body.roomId}`);
            }
            ...
            if (member) {
                broadcastSystemMessage(body.roomId, `${member.displayName} 加入了房间`);
            }
            // Check if room is empty and auto-play
            if (rec && !rec.nowPlaying && rec.queue.length === 0) {
                console.log(`[Join] Room ${body.roomId} is empty, triggering auto-play...`);
    ...
    socket.on("disconnect", () => {
        try {
            ...
            if (userId && roomId) {
                console.log(`[Disconnect] User ${userId} disconnected from room ${roomId}, scheduling removal...`);
                scheduleLeave(roomId, userId, () => {
                    console.log(`[Disconnect] Removing user ${userId} from room ${roomId} after timeout`);
                    try {
                        const rec = rooms.get(roomId);
                        const member = rec?.members.get(userId);
                        if (member) {
                            broadcastSystemMessage(roomId, `${member.displayName} 离开了房间`);
                        }
