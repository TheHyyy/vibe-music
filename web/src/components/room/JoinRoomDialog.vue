...
import { User, Sparkles, KeyRound } from "lucide-vue-next";
import { ElMessage } from "element-plus";
import Button from "@/components/ui/Button.vue";
import Input from "@/components/ui/Input.vue";
import { getPublicRoomInfo, joinRoomById } from "@/api/rooms";
...
const form = reactive({
  displayName:
    localStorage.getItem("echo_username") ||
...
async function onJoin() {
  ...
  if (roomInfo.value.hasPassword && !inviteToken && !form.password) {
  ...
  try {
    const res = await joinRoomById(props.roomId, {
...
<template>
  ...
  >
    ...
    >
      ...
      <div
        v-if="loading"
        class="flex h-64 flex-col items-center justify-center gap-4 text-slate-400"
      >
        <div
      ...
      <div v-else-if="roomInfo" class="p-6">
        <div class="mb-8 text-center">
          <div
            class="mx-auto mb-4 flex h-12 w-12 items-center justify-center rounded-xl bg-indigo-500/20 text-indigo-400"
          >
          ...
          <p class="mt-1 text-sm text-slate-400">
            ...
            <span class="text-indigo-400"
              >@{{ roomInfo.hostName || "Unknown" }}</span
            >
          </p>
        </div>
        ...
        <div class="space-y-4">
          ...
          <div
            v-if="roomInfo.hasPassword && !route.query.invite"
            class="space-y-1.5"
          >
            <label class="text-xs font-medium text-slate-400 ml-1"
              >房间密码</label
