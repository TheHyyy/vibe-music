...
const canEdit = computed(() => role.value === "HOST" || role.value === "MODERATOR");
  ...
  <div class="glass flex flex-col rounded-2xl">
    ...
    <div class="space-y-3 p-3">
      <div v-if="canEdit" class="flex items-center gap-2">
        <Input v-model="input" placeholder="输入用户 ID" class="bg-slate-900/50" />
        <Button size="sm" variant="danger" @click="add">拉黑</Button>
      </div>

      <div v-if="mock.length === 0" class="py-4 text-center text-xs text-slate-500">
        暂无黑名单记录
      </div>
      ...
      <div v-else class="space-y-1">
        ...
        >
          ...
          <Button v-if="canEdit" size="icon" variant="ghost" class="h-6 w-6 text-slate-400 hover:text-red-400" @click="remove(v)">
